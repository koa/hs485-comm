...
public class PacketEncoder implements Closeable {
	...
	synchronized public void sendRawPacket(final boolean longPacket, final byte[] rawPacketData) throws IOException {
		...
		long sleepTime = decoder.getLastPacketReceived() - System.currentTimeMillis() - minWaitTime;
		while (sleepTime > 0) {
			try {
			...
			sleepTime = System.currentTimeMillis() - decoder.getLastPacketReceived() - minWaitTime;
		}
